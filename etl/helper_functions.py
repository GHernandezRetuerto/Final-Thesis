    ...
    with open(output_file, 'r') as f:
        for i, l in enumerate(f):
            if i == 0:
                ...
                probabilities = tuple(
                    float(entry) if is_finite_number(entry) else float('nan') for entry in l.split(','))
            else:
                break
...
# Load a trained model. This function is *required*. You should edit this function to add your code, but do *not*
...
def get_features(header, recording, leads):
    ...
    for i in range(num_leads):
        ...
        # Probar otras estrategias: media, mediana,...
